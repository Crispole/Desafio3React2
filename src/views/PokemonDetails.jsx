  ...
  return (
    <>
      ...
      <div className='baseCenter'>
        <h1>{pkmn.name[0] + pkmn.name.substring(1)}</h1>
        <h2>Pokémon #{id}</h2>
        {pkmn.types.length < 2 ?
        (<h3>Type:{' ' + pkmn.types[0].type.name[0] + pkmn.types[0].type.name.substring(1)}</h3>)
        :
        (<h3>Types:
          {' ' + pkmn.types[0].type.name[0] + pkmn.types[0].type.name.substring(1) + ' '}
          /
          {' ' + pkmn.types[1].type.name[0] + pkmn.types[1].type.name.substring(1)}
        </h3>)
        }
